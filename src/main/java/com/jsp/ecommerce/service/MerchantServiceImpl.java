...
public class MerchantServiceImpl implements MerchantService {
	...
	@Override
	public String loadAddProduct(MerchantProductForm productDto, Model model, HttpSession session) {
		Merchant merchant = (Merchant) session.getAttribute("merchant");
		if (merchant != null) {
	...
	@Override
	public String addProduct(@Valid MerchantProductForm productDto, BindingResult result, HttpSession session,
			Model model) {
		Merchant merchant = (Merchant) session.getAttribute("merchant");
		if (merchant != null) {
	...
	public String editProduct(Long id, Model model, HttpSession session) {
		...
		if (merchant != null) {
			...
			MerchantProductForm productDto = new MerchantProductForm();
	...
	@Override
	public String updateProduct(Long id, MerchantProductForm productDto, BindingResult result, Model model,
			HttpSession session) {
		Merchant merchant = (Merchant) session.getAttribute("merchant");
		if (merchant != null) {
			...
			product.setCategory(productDto.getCategory());
			try {
				if (productDto.getImage() != null && productDto.getImage().getInputStream().available() > 0) {
					product.setImageUrl(cloudinaryHelper.saveImage(productDto.getImage()));
				}
			} catch (IOException e) {
				e.printStackTrace();
